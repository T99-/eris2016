...
public class VisionSystem2 {
	...
	public void init() {
		...
		if (RobotMap.shooterAngleServo != -1) angleServo = new Servo(RobotMap.shooterAngleServo); 

		if (RobotMap.shooterTiltServo != -1) tiltServo = new Servo(RobotMap.shooterTiltServo);
	...
	public void shooterAngle(Joystick shooterStick) {
		...
		if (shooterStick.getRawButton(RobotMap.cameraAngleRightButton)) angleServo.setAngle(servoAngle - 1);

		//Angles the camera left
		if (shooterStick.getRawButton(RobotMap.cameraAngleLeftButton)) angleServo.setAngle(servoAngle + 1);

		//Centers the camera
		if (shooterStick.getRawButton(RobotMap.cameraCenterViewButton)) angleServo.setAngle(90);
